...
function init() {
   ...
   //Insert the HTML code for the fiurst puzzle table
...
if(confirm("You will lose all of your work on the puzzle! Continue?")) { 
   ...
   function setupPuzzle() {
      ...
      for(var i = 0; i < puzzleCells.length; i++) {
               puzzleCells[i].style.background = "rgb(233,207,29)";
      // set the cell background color in response to the mousedown event
               puzzleCells[i].onmousedown = setBackground; 
      // Usw a pencil image as the cursor
      ...
      //Create an event listener to hihjligh incorrect cells
         ...
         for(var i = 0; i < puzzleCells.length; i++) {
            if((puzzleCells[i].className === "filled" && 
               ...
               (puzzleCells[i].className === "empty" &&
                puzzleCells[i].style.backgroundColor === "rgb(101, 101, l01)")) {
...
function setBackground(e) {
   ...
   if (e.shiftKey) {
      ...
      cellBackground = "rgb(101, 101, 101)"; //standard gray
   ...
   // Prevent the default acrion of selecting table heading text
   e.preventDefault();
